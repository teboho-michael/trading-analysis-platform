...
import ForwardTestJournal from "../Journal/ForwardTestJournal";

const TABS = ["Signals", "Zones", "Forward Test Journal", "Performance", "Alerts", "System"];
...
export default function LowerTabs({ selectedSymbol, collapsed, onToggleCollapsed, journalRefreshToken }) {
  ...
  return <section className={`lower-panel panel-shell${collapsed ? " collapsed" : ""}`}>
    <nav className="lower-tabs" aria-label="Analysis history">{TABS.map((name) => <button key={name} type="button" className={tab === name ? "active" : ""} onClick={() => setTab(name)}>{name}</button>)}<button type="button" className="lower-collapse" onClick={onToggleCollapsed}>{collapsed ? "Expand" : "Collapse"}</button></nav>
    {!collapsed && <div className="lower-content">{error && <p>{error}</p>}
